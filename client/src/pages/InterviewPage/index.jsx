...
function InterviewPage() {
    ...
    const loadInterview = async () => {
      try {
        ...
        if (data.questions && data.questions.length > 0) {
          ...
          const loadedQuestion = data.questions[qIndex] || data.questions[0];
          setCurrentQuestion(loadedQuestion);
          if (loadedQuestion?.codeLanguage) {
            setCodeLanguage(loadedQuestion.codeLanguage);
          }
        }
  ...
  const processAnswerResult = (result) => {
    ...
    if (result.question?.codeLanguage) {
      setCodeLanguage(result.question.codeLanguage);
    }
    setCurrentAudio(result.audio);
    setAudioKey((prev) => prev + 1);
    resetAnswerFields();
  ...
  const handleSubmitCode = async (codeOverride) => {
    const submittedCode = typeof codeOverride === 'string' ? codeOverride : code;
    ...
    setInterviewerState(STATE_THINKING);
    try {
      const result = await submitCode(id, submittedCode, codeLanguage);
                        ...
                        <VoiceRecorder
                          onRecordingComplete={async (audioBlob) => {
                            ...
                            try {
                              ...
                              setCode(text);
                              await handleSubmitCode(text);
                            } catch (error) {
                              const fallbackText =
                                'Verbal explanation provided.';
                              setCode(fallbackText);
                              await handleSubmitCode(fallbackText);
                            }
                          }}
                          disabled={submitting}
                        />
